...
> [!IMPORTANT]
> Ваш код **должен** быть основан на предоставленном Project Template. Разрешается выбрать любую ветку как начальную точку (может быть, ветка `main` покажется проще, чем `ASR`). Однако мы настоятельно рекомендуем ветку `ASR` для этой домашней работы.
