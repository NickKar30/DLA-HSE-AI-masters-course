# DLA-HSE-AI-masters-course

Репозиторий курса **Deep Learning for Audio** для магистратуре НИУ ВШЭ "Искусственный интеллект".

## Расписание лекций

| Номер | Тема | Ссылка на GitHub |
| :--- | :--- | :--- |
| 1 | Digital Signal Processing | [Слайды](lectures/lec1_dsp.pdf) |
| 2 | Speech Processing Basics | [Слайды](lectures/lec2_speech.pdf) |
| 3 | Automatic Speech Recognition | [Слайды](lectures/lec3_asr.pdf) |
| 4 | Audio Understanding & Language Models | [Слайды](lectures/lec4_audio_understanding_lm.pdf) |
| 5 | VAD & Source Separation | [Слайды](lectures/lec5_vad_separation.pdf) |
| 6 | Acoustic Models | [Слайды](lectures/lec6_acoustic_models.pdf) |
| 7 | Vocoders & End-to-End Models | [Слайды](lectures/lec7_vocoders_e2e.pdf) |
| 8 | Neural Audio Codecs | [Слайды](lectures/lec8_codecs.pdf) |